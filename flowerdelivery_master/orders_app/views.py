...
from flowerdelivery_master.telegram_bot_app.bot import notify_new_order, notify_order_status
    ...
    if request.method == 'POST':
        ...
        for flower_id, quantity in cart.items():
            ...
            await sync_to_async(OrderItem.objects.create)(
                order=order, flower=flower, quantity=quantity
            )

        # Очистка корзины
        request.session['cart'] = {}
        messages.success(request, f"Ваш заказ №{order.pk} успешно оформлен!")

        # Уведомление через Telegram
        await notify_new_order(order.pk)
...
def update_catalog_item_in_catalog(request, flower_id):
    ...
    if request.method == 'POST':
        ...
        if str(flower_id) not in cart:
            cart[str(flower_id)] = 0

        # Обрабатываем действие (увеличить или уменьшить количество)
        action = request.POST.get('action')
        if action == 'plus':
            cart[str(flower_id)] += 1
        elif action == 'minus':
